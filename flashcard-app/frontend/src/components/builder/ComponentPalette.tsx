...
export const ComponentPalette: React.FC<ComponentPaletteProps> = ({ onComponentDragStart }) => {
  ...
  const groupedComponents = () => {
    ...
    const layoutComponents: ComponentRegistryEntry[] = [];
    const otherComponents = new Map<string, ComponentRegistryEntry[]>();

    filtered.forEach(comp => {
      const category = comp.category || 'Other';

      // Separate layout components
      if (category.toLowerCase() === 'layout') {
        layoutComponents.push(comp);
      } else {
        if (!otherComponents.has(category)) {
          otherComponents.set(category, []);
        }
        otherComponents.get(category)!.push(comp);
      }
    });

    return { layoutComponents, otherComponents };
  };

  const categories = getCategories();
  const { layoutComponents, otherComponents } = groupedComponents();
    ...
    <div className="component-palette">
      ...
      {!isLoading && !error && (
        <div className="component-list">
          {layoutComponents.length === 0 && otherComponents.size === 0 ? (
            ...
            <>
              ...
              {layoutComponents.length > 0 && (
                <div className="component-category layout-category">
                  <div
                    className="category-header"
                    onClick={() => toggleCategoryExpansion('layout')}
                  >
                    <span className="category-toggle">
                      {expandedCategories.has('layout') ? '▼' : '▶'}
                    </span>
                    <h4>Layouts</h4>
                    <span className="category-count">{layoutComponents.length}</span>
                  </div>

                  {expandedCategories.has('layout') && (
                    <div className="category-components">
                      <div className="layout-hint">
                        Add a layout first, then add UI components inside
                      </div>
                      {layoutComponents.map(component => (
                        <div
                          key={`${component.pluginId}-${component.componentId}`}
                          className="component-item layout-item"
                          draggable
              ...
              {Array.from(otherComponents.entries()).map(([category, comps]) => (
