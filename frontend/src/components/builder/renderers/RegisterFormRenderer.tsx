...
const RegisterFormRenderer: React.FC<RendererProps> = ({ component, isEditMode }) => {
  ...
  const validatePassword = (password: string): string[] => {
    const errors: string[] = [];
    const minLength = props.passwordMinLength || 8;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(props.registerEndpoint || '/api/auth/register', {
        method: 'POST',
      ...
      if (response.ok) {
        window.location.href = props.redirectUrl || '/login';
      } else {
      ...
      <form onSubmit={handleSubmit} className="auth-form-content">
        ...
        {props.showFullName !== false && (
          <div className="auth-form-field">
            <label htmlFor="fullName">{props.fullNameLabel || 'Full Name'}</label>
            <input
              type="text"
              id="fullName"
        ...
        <div className="auth-form-field">
          <label htmlFor="email">{props.emailLabel || 'Email'}</label>
          <input
            type="email"
            id="email"
        ...
        {props.showUsername !== false && (
          <div className="auth-form-field">
            <label htmlFor="username">{props.usernameLabel || 'Username'}</label>
            <input
              type="text"
              id="username"
        ...
        {props.showConfirmPassword !== false && (
          <div className="auth-form-field">
            <label htmlFor="confirmPassword">
              {props.confirmPasswordLabel || 'Confirm Password'}
            </label>
            <input
              type="password"
              id="confirmPassword"
        ...
        {props.showTermsCheckbox !== false && (
          <div className="auth-form-options">
            <label className="auth-checkbox">
              ...
              />
              <span>
                {props.termsText || 'I agree to the'}{' '}
                <a
                  href={props.termsUrl || '/terms'}
        ...
        <button
          ...
          disabled={isEditMode || isLoading}
        >
          {isLoading ? 'Creating account...' : props.submitButtonText || 'Create Account'}
        </button>

        {props.showLoginLink !== false && (
          <p className="auth-form-footer">
            {props.loginText || 'Already have an account?'}{' '}
            <a
              href={props.loginUrl || '/login'}
              className="auth-form-link"
              onClick={(e) => isEditMode && e.preventDefault()}
            >
              {props.loginLinkText || 'Sign in'}
            </a>
          </p>
        )}
      </form>
