...
const ForgotPasswordFormRenderer: React.FC<RendererProps> = ({ component, isEditMode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(props.resetEndpoint || '/api/auth/forgot-password', {
        method: 'POST',
        ...
        <div className="auth-success">
          ...
          {props.showBackToLogin !== false && (
            <p className="auth-form-footer">
              <a
                href={props.backToLoginUrl || '/login'}
                className="auth-form-link"
                onClick={(e) => isEditMode && e.preventDefault()}
              >
                ← {props.backToLoginLinkText || 'Back to Sign in'}
              </a>
            </p>
          )}
        </div>
    ...
    <div className="auth-form forgot-password-form" style={containerStyle}>
      ...
      <form onSubmit={handleSubmit} className="auth-form-content">
        ...
        <div className="auth-form-field">
          <label htmlFor="email">{props.emailLabel || 'Email Address'}</label>
          <input
            type="email"
            id="email"
        ...
        <button
          ...
          disabled={isEditMode || isLoading}
        >
          {isLoading ? 'Sending...' : props.submitButtonText || 'Send Reset Link'}
        </button>

        {props.showBackToLogin !== false && (
          <p className="auth-form-footer">
            {props.backToLoginText || 'Back to'}{' '}
            <a
              href={props.backToLoginUrl || '/login'}
              className="auth-form-link"
              onClick={(e) => isEditMode && e.preventDefault()}
            >
              {props.backToLoginLinkText || 'Sign in'}
            </a>
          </p>
        )}
      </form>
